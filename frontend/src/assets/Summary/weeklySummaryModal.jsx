...
export default function WeeklySummaryModal({ isOpen, onClose, apiBaseUrl }) {
  ...
  const generateWeeklySummary = (records) => {
    ...
    records.forEach((r) => {
      ...
      const fullDate = r.dateInfo?.fullDate || "N/A";
  ...
  // Fetch weekly records + AI summary
  const fetchWeeklyRecords = async () => {
    ...
    try {
      ...
      // 4️⃣ AI summary with safe handling
      try {
        ...
        if (ragRes.ok) {
          ...
          setAiSummary(ragData.summary || null);
        } else {
      ...
      <div className="bg-white rounded-xl w-full max-w-2xl p-6 relative max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input type="number" min="1" max="53" value={week} onChange={(e) => setWeek(e.target.value)} className="w-full border rounded-lg px-3 py-2" />
          </div>
        </div>
          ...
          <div className="mt-6 flex-1 overflow-y-auto space-y-4 text-sm">
            ...
            <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-4 rounded-lg">
              ...
              {aiLoading ? (
                ...
                <p className="text-sm italic text-purple-200">AI summary not available</p>
              )}
            </div>
            ...
            {/* Folder Usage */}
            <div className="bg-white p-3 rounded border">
              <p><strong>Folder Usage:</strong> {summary.folderUsage?.withFolder || 0} in folders, {summary.folderUsage?.withoutFolder || 0} not in folders</p>
            </div>

            {/* Latest Record */}
            <div className="bg-white p-3 rounded border">
              <p><strong>Latest Record:</strong> {summary.latestRecord || "N/A"}</p>
            </div>

            {/* Records by Day */}
            <div className="bg-white p-3 rounded border">
              <strong className="block mb-3">Records by Day:</strong>
              {Array.isArray(summary.recordsByDay) && summary.recordsByDay.length > 0 ? (
                summary.recordsByDay.map(({ day, date, records, categories }) => (
                  <div key={day} className="p-3 bg-blue-50 rounded border-l-4 border-blue-500 mb-2">
                    <p className="font-semibold text-blue-700 mb-2">{day}, {date} <span className="ml-2 text-xs text-gray-600">({records?.length || 0} record{(records?.length || 0) !== 1 ? 's' : ''})</span></p>

                    {/* Categories */}
                    {categories && Object.keys(categories).length > 0 && (
                      <div className="mb-2 text-sm">
                        {Object.entries(categories).map(([cat, count]) => (
                          <span key={cat} className="inline-block mr-2 mb-1 px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">{cat} ({count})</span>
                        ))}
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5">
                      {Array.isArray(records) && records.map((record, idx) => (
                        <div key={idx} className="text-sm pl-3 py-1 border-l-2 border-blue-300 bg-white rounded">
                          ...
                          {record.category && (
                            <span className="ml-2 text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded">{record.category}</span>
                          )}
                        </div>
                      ))}
                    </div>
